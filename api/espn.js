  ...
  try {
    ...
    if (!response.ok) {
      return res.status(502).json({
        ok: false,
        error: `ESPN returned HTTP ${response.status}. Your ESPN session cookies may need to be refreshed.`,
        teams: []
      });
    }
    ...
    const members = Object.fromEntries(
      (data.members || []).map((m) => {
        ...
        return [m.id, managerNames[espnName] || espnName];
      })
    );
    ...
    const teams = (data.teams || [])
      .map((t) => ({
        ...
        pointsFor:
          Math.round((t.record?.overall?.pointsFor ?? 0) * 100) / 100,
        pointsAgainst:
          Math.round((t.record?.overall?.pointsAgainst ?? 0) * 100) / 100
      }))
      .sort(
        (a, b) =>
          b.wins - a.wins ||
    ...
    const matchups = (data.schedule || [])
      ...
      .map((g) => ({
        ...
        complete:
          (g.home?.totalPoints ?? 0) > 0 ||
          (g.away?.totalPoints ?? 0) > 0
      }));
    ...
    return res.status(200).json({
      ...
      leagueId,
      season,
      name: data.settings?.name || "Crippling Gambling Addicts",
      teams,
      matchups
    });
  } catch (error) {
    return res.status(500).json({
      ok: false,
      error: "Unable to reach ESPN from the server.",
      teams: []
    });
  }
}
